...
class Agent(Base_Agent):
    def __init__(self, host: str, agent_port: int, monitor_port: int, unum: int,
        ...
        self.state = 0  # 机器人状态: 0-正常, 1-起身, 2-向左扑救, 3-向右扑救, 4-等待
    ...
    def think_and_send(self):
        ...
        if w.play_mode in [NeuMode.BEFORE_KICKOFF, TheirMode.GOAL, OurMode.GOAL]:
        ...
        elif self.state == 2:  # 执行向左扑救
            # 扑救完成后进入等待状态
            self.state = 4 if self.behavior.execute("Dive_Left") else 2
        elif self.state == 3:  # 执行向右扑救
            # 扑救完成后进入等待状态
            self.state = 4 if self.behavior.execute("Dive_Right") else 3
        elif self.state == 4:  # 扑救后或对方开球时等待
            pass
        elif self.state == 1 or self.behavior.is_ready("Get_Up"):  # 起身或已倒地
            # 起身动作完成后恢复正常状态
            ...
            if ball_2d[0] < -10:  # 当球靠近球门时，执行扑救
                self.state = 2 if ball_2d[1] > 0 else 3
        else:  # 前锋逻辑
