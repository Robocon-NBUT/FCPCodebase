  ...
  public:
    ...
    struct sMarker
    {
        ...
        sMarker(const sFieldPoint *fieldPt_, const Vector3 &relPosPolar_)
            : absPos(fieldPt_->pt), fieldPt(fieldPt_), relPosPolar(relPosPolar_),
        ...
        sMarker(const Vector3 &absPos_, const Vector3 &relPosPolar_)
            : absPos(sVector3d({absPos_.x, absPos_.y, absPos_.z})), relPosPolar(relPosPolar_),
              relPosCart(relPosPolar_.toCartesian()){};
        ...
        /**
         * Constructor with field segment, absolute position, relative polar & cartesian coordinates (e.g. unknown
         * marker)
         */
        sMarker(const sFieldSegment *fieldSeg_, const sVector3d &absPos_, const Vector3 &relPosPolar_,
                const Vector3 &relPosCart_)
    ...
    static constexpr float var_round_hundredth =
        0.01 * 0.01 / 12; // variance of uniformly distributed random variable [-0.005,0.005]
    ...
    class cFieldPoints
    {
      public:
        ...
        static constexpr std::array<sFieldPoint, 28> list{{
            {-cHalfFielfLength, -cHalfGoalWidth, cGoalHeight, "post--"},
            {-cHalfFielfLength, cHalfGoalWidth, cGoalHeight, "post-+"}, // Goalposts x<0
            {cHalfFielfLength, -cHalfGoalWidth, cGoalHeight, "post+-"},
            {cHalfFielfLength, cHalfGoalWidth, cGoalHeight, "post++"}, // Goalposts x>0
            {-cHalfFielfLength, -cHalfFieldWidth, 0, "corner--"},
            {-cHalfFielfLength, cHalfFieldWidth, 0, "corner-+"}, // Corners x<0
            {cHalfFielfLength, -cHalfFieldWidth, 0, "corner+-"},
            {cHalfFielfLength, cHalfFieldWidth, 0, "corner++"}, // Corners x>0
            {0, -cHalfFieldWidth, 0, "halfway-"},               // Halfway line ending y<0
            {0, cHalfFieldWidth, 0, "halfway+"},                // Halfway line ending y>0
            {-cHalfFielfLength, -cHalfPenaltyWidth, 0, "boxBack--"},
            {-cHalfFielfLength, cHalfPenaltyWidth, 0, "boxBack-+"}, // Penalty box goal line corner x<0
            {cHalfFielfLength, -cHalfPenaltyWidth, 0, "boxBack+-"},
            {cHalfFielfLength, cHalfPenaltyWidth, 0, "boxBack++"}, // Penalty box goal line corner x>0
            {-cPenaltyBoxDistX, -cHalfPenaltyWidth, 0, "boxFrnt--"},
            {-cPenaltyBoxDistX, cHalfPenaltyWidth, 0, "boxFrnt-+"}, // Penalty box front corner x<0
            {cPenaltyBoxDistX, -cHalfPenaltyWidth, 0, "boxFrnt+-"},
            {cPenaltyBoxDistX, cHalfPenaltyWidth, 0, "boxFrnt++"}, // Penalty box front corner x>0
            {2, 0, 0, "r0"},
            {1.6180339887498948, 1.1755705045849463, 0, "r36"}, //(18,19) Ring 0/36 deg
            {0.61803398874989485, 1.9021130325903071, 0, "r72"},
            {-0.61803398874989485, 1.9021130325903071, 0, "r108"}, //(20,21) Ring 72/108 deg
            {-1.6180339887498948, 1.1755705045849463, 0, "r144"},
            {-2, 0, 0, "r180"}, //(22,23) Ring 144/180 deg
            {-1.6180339887498948, -1.1755705045849463, 0, "r216"},
            {-0.61803398874989485, -1.9021130325903071, 0, "r252"}, //(24,25) Ring 216/252 deg
            {0.61803398874989485, -1.9021130325903071, 0, "r288"},
            {1.6180339887498948, -1.1755705045849463, 0, "r324"} //(26,27) Ring 288/324 deg
        }};
    ...
    class cFieldLineSegments
    {
      public:
        ...
        static constexpr std::array<sFieldSegment, 21> list{{
            ...
            {&cFieldPoints::boxgoal_mm, &cFieldPoints::box_mm, cPenaltyLength, c0deg,
             "box--"}, // Penalty box sideline x<0 y<0
            {&cFieldPoints::boxgoal_mp, &cFieldPoints::box_mp, cPenaltyLength, c0deg,
             "box-+"}, // Penalty box sideline x<0 y>0
            {&cFieldPoints::box_pm, &cFieldPoints::boxgoal_pm, cPenaltyLength, c0deg,
             "box+-"}, // Penalty box sideline x>0 y<0
            {&cFieldPoints::box_pp, &cFieldPoints::boxgoal_pp, cPenaltyLength, c0deg,
             "box++"}, // Penalty box sideline x>0 y>0
     ...
     * It's different from world.landmarks since it is ordered by position, not by name, and it holds the cartesian
     * relPos (this ordering difference is important when the teams switch sides)
     */
    ...
    static inline double normalize_line_angle_deg(double deg)
    {
        return 90.0 - abs(fmod(abs(deg), 180.0) - 90.0);
    }

    static inline double normalize_line_angle_rad(double rad)
    {
        return numbers::pi / 2 - abs(fmod(abs(rad), numbers::pi) - numbers::pi / 2);
    }

    static inline double normalize_vector_angle_deg(double deg)
    {
        return 180.0 - abs(fmod(abs(deg), 360.0) - 180.0);
    }

    static inline double normalize_vector_angle_rad(double rad)
    {
        return numbers::pi - abs(fmod(abs(rad), numbers::pi * 2) - numbers::pi);
    }
};
