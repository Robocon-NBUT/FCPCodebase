...
void LocalizerV2::run()
{
    ...
    {
        if (lines_no == 0 && landmarks_no == 0)
        {
            stats_change_state(BLIND);
        }
        else
        {
            stats_change_state(MINFAIL);
        }
        return;
    }
    ...
    if (!(landmarks_no > 1 ? find_xy() : guess_xy()))
    {
        return;
    }
...
void add_gsl_regression_sample(gsl_matrix *m, gsl_vector *v, int sample_no, const Vector3 &relativeCoord,
                               double absoluteCoord, double translCoeffMult = 1)
{
...
template <std::size_t SIZE> gsl_vector *create_gsl_vector(const std::array<double, SIZE> &content)
{
...
Vector get_ground_unit_vec_perpendicular_to(const Vector3 &vec)
{
    ...
    if (aux > 0)
    { // if vec is (0,0,1) or (0,0,-1) we keep the default rotation axis
        gx = -vec.y / aux;
        gy = vec.x / aux;
    }
...
Vector3 fast_rotate_around_ground_axis(Vector3 v, Vector3 Zvec)
{
    ...
    // assert(cos_a <= 1);
    if (cos_a > 1)
        cos_a = 1;                           // Fix: it happens rarely, no cause was yet detected
...
void fast_compute_XYvec_from_Zvec(const Vector3 &Zvec, float agent_angle, Vector3 &Xvec, Vector3 &Yvec)
{
     ...
     * (whose seen normal vector is (0,0,1)). We need: axis of rotation (unit vector lying on XY plane) and angle
     * (rads): rotation axis: u = (0,0,1)x(Zvec) (vector perpendicular to XY plane and Zvec) = (-ZvecY,ZvecX,0)
     * (rotation will be counterclockwise when u points towards the observer) (so a negative angle will bring the agent
     * to the ROTsys) angle between Zvec and (0,0,1): a = acos( ((0,0,1).(Zvec)) / (|(0,0,1)|*|Zvec|) ) =	acos(
     * ((0,0,1).(Zvec)) ) =	acos( ZvecZ )
     *
     ...
     * 		where R is: (rotation matrix from axis and angle
     * https://en.wikipedia.org/wiki/Rotation_matrix#Rotation_matrix_from_axis_and_angle) R00 = cos(a) + ux*ux(1-cos(a))
     * R01 = ux*uy(1-cos(a)) R10 = uy*ux(1-cos(a))             R11 = cos(a) + uy*uy(1-cos(a)) R20 = -uy*sin(a) R21 =
     * ux*sin(a) so Xvec becomes: XvecX = cos(a)*cos(b) + (1-cos(a))*(ux*ux*cos(b) + ux*uy*sin(b)) XvecY = cos(a)*sin(b)
     * + (1-cos(a))*(uy*uy*sin(b) + ux*uy*cos(b)) XvecZ = sin(a)*(ux*sin(b) - uy*cos(b))
     *
...
bool LocalizerV2::find_z_axis_orient_vec()
{
    ...
    if (l->length < 1)
    { // larger line is too short
        stats_change_state(FAILzLine);
        return false;
    }

    if (goalNo == 0)
    { // Case e.g.: 1 corner and 1 line (or 2 short lines)
    ...
    if (abs(possibleZvecLength - 0.8) < 0.05)
    { // Solution A & B
    ...
    Vector3 crossbar_left_vec, crossbar_midp; // this crossbar vector points left if seen from the midfield (this is
                                              // important for the cross product)
...
void LocalizerV2::fit_ground_plane()
{
    ...
    for (const auto &g : ground_markers)
    { // Insert all weighted groundmarks in matrix
    ...
    // Note: |d| is an estimate of the agent's height, but we can do better by including aerial references in the
    // prediction later
     ...
     * To do that, we check if the origin of the relative coordinate system (agent's head) gives a positive sign for (ax
     *+ by + cz - d) ax + by + cz - d  > 0   (replacing x=0, y=0, z=0) d < 0
     *
    ...
    if (!fd.list_landmarks_goalposts.empty())
    {                                                                             // If there are visible goal posts
        const Vector3 &aerialpt = fd.list_landmarks_goalposts.front().relPosCart; // random aerial point (goal post)
        if (a * aerialpt.x + b * aerialpt.y + c * aerialpt.z < d)
        { // the goalpost is on the negative side, so we invert the normal vector
            a = -a;
            b = -b;
            c = -c;
        }
    ...
    { // If there are no visible goal posts, we rely on the agent's head
        if (d > 0)
        { // the normal vectors points down, so we invert it (we ignore 'd' from this point forward)
            a = -a;
            b = -b;
            c = -c;
        }
...
double LocalizerV2::map_error_2d(const gsl_vector *v, void *params)
{
    ...
    for (const Line6 &l : fd.list_segments)
    {
        ...
        if (l.length > 0.8)
        {
            // This is the easy case: find the angle and establish a small tolerance (which allows some visual rotation
            // attempts)
            l_angle = atan2f(le.y - ls.y, le.x - ls.x);
            if (l_angle < 0)
            {
                l_angle += 3.14159265f;
            } // this is a line, not a vector, so positive angles are enough
        ...
        else if (fd.list_segments.size() <= 3)
        {
            ...
            // If the small line is touching a big line, they have different orientations (it's a characteristic from
            // the field lines)

            for (const Line6 &lbig : fd.list_segments)
            {
                if (lbig.length < 2 || &lbig == &l)
                    continue; // check if line is big and different from current

                if (lbig.segmentDistToSegment(l) < 0.5)
                {
                    // this would only generate false positives with the halfway line and 4 ring lines (if enough vision
                    // error) but even then, their orientation would be very different, so the method still holds

                    //---------- get angle perpendicular to bigline (that is either the small line's angle, or at least
                    // close enough)
                    ...
                    // This large tolerance means that this small line can be matched with almost everything except
                    // perpendicular lines
        ...
        // this default error of 1e6f is applied when there is no match (which means the transf. matrix's Xvec/Yvec are
        // wrong)
        float min_err = 1e6f;
        for (const auto &s : Field::cFieldLineSegments::list)
        { // find distance to closest field line

            // Skip field line if seen line is substantially larger
            if (l.length > (s.length + 0.7))
            {
                continue;
            }
            ...
            if (angle_difference > 1.57079632f)
                angle_difference = 3.14159265f - angle_difference;
            if (angle_difference > l_angle_tolerance)
                continue;
            ...
            if (err < min_err)
                err += Field::fieldLineSegmentDistToCart2DPoint(s, le.to2d());
...
bool LocalizerV2::fine_tune(float initial_angle, float initial_x, float initial_y)
{
    ...
    counter_fineTune += stats_sample_position_error(Vector3(initial_x, initial_y, prelimHeadToField.get(11)),
                                                    world.my_cheat_abs_cart_pos, errorSum_fineTune_before);

    // Fine tune, changing the initial parameters directly
    if (!fine_tune_aux(initial_angle, initial_x, initial_y, false))
        return false;

    // Statistics for 1st fine tune
    stats_sample_position_error(Vector3(initial_x, initial_y, prelimHeadToField.get(11)), world.my_cheat_abs_cart_pos,
    ...
    stats_sample_position_error(prelimHeadToField.toVector3(), world.my_cheat_abs_cart_pos,
...
bool LocalizerV2::fine_tune_aux(float &initial_angle, float &initial_x, float &initial_y, bool use_probabilities)
{
    ...
    if (use_probabilities)
        minex_func.f = map_error_logprob; // probablity-based error function
    ...
    do
    {
        ...
        status = gsl_multimin_test_size(
            size,
            1e-3); // This size can be used as a stopping criteria, as the simplex contracts itself near the minimum
...
bool LocalizerV2::guess_xy()
{
    ...
    do
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (current_error[i] < lowest_error)
                lowest_error = current_error[i];
            ...
            status = gsl_multimin_test_size(
                size,
                1e-2); // This size can be used as a stopping criteria, as the simplex contracts itself near the minimum

            if (status != GSL_CONTINUE || (lowest_error * 50 < current_error[i]))
            { // finished or aborted
    ...
    for (int i = 0; i < 4; i++)
    {
        if (!plausible_solution[i])
            continue;
        bool isDistanceOk =
            (!noLandmarks) || last_known_position.distance(best_xy[i]) < 0.5; //  distance to last known position
        if (current_error[i] < 0.12 && isDistanceOk)
        { // mapping error
            plausible_count++;
            last_i = i;
        }
    ...
    if (!noLandmarks && plausible_count > 1)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            if (plausible_solution[i] && last_known_position.distance(best_xy[i]) < 0.5)
    ...
    else if (current_error[last_i] > 0.06 || (noLandmarks && last_known_position.distance(best_xy[last_i]) > 0.3))
    { // mapping error  /  distance to last known position
...
void LocalizerV2::commit_everything()
{
    ...
    final_headTofieldTransform.inverse_tranformation_matrix(
        final_fieldToheadTransform); // Full transformation (absolute to relative)
    ...
    if (position_history_ptr >= position_history.size())
        position_history_ptr = 0;
}
...
{
    // assert(n > 0 && n < position_history.size() && "LocalizerV2::get_velocity(unsigned int n) -> n must be between 1
    // and 9!");
...
void LocalizerV2::print_report() const
{
    ...
    if (counter_fineTune < 2)
        return; // otherwise, c-1=0
...
int LocalizerV2::stats_sample_position_error(const Vector3 sample, const Vector3 &cheat, double error_placeholder[])
{
    ...
    if (world.my_cheat_abs_cart_pos == Vector3(0, 0, 0))
        return 0;
