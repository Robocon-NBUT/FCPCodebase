 ...
 *
 * WARNING:
 * When partial information is available, it is used to change the head position (e.g. translation in x/y/z may be
 * updated without visual information). However, the transformation matrix (including the translation) are not changed,
 * because this would affect the perceived position of previously seen objects. For this reason, the worldstate should
 * not rely on the head position to convert relative to absolute coordinates. Instead, it should only use transformation
 * matrix, or internal conversion methods. The head position can still be used for other purposes, such as machine
 * learning.
 *
 * -----------------------------------------------------------------------------------
 ...
 *  					If we see one and we are on top of it, the feet contact points can cause collinearity but it is
 * very rare. SOLUTION: Find the best fitting ground plane's normal vector using Singular Value Decomposition
 *
 ...
 * 			1.2.B. IF IT IS THE GOALLINE. Find the line's nearest point (p) to the goalposts (g1,g2) midpoint (m), Zvec
 * = (m-p) / |Zvec| (This solution is more accurate than using only 1 goalpost. Even m is more accurate, on average,
 * than g1 or g2.) 1.2.C. IF IT IS NOT THE GOALLINE. There are 3 options: I - There are 2 goalposts (crossbar line) and
 * an orthogonal line: Zvec = crossbar x line (or) line x crossbar (depending of the direction of both vectors) II -
 * Other situation if the z translation coordinate was externally provided through machine learning: Find any horizontal
 * line (e.g. line between 2 goalposts, or ground line) Let M be any mark with known absolute z, and let Z be the
 * externally provided z coordinate: Find Zvec such that (HorLine.Zvec=0) and (Zvec.Mrel=Mabsz-Z) III - If z was not
 * provided: Skip to last step. 1.2.*. This scenario was tested and it is not valid. In certain body positions there are
 * two solutions, and even though one is correct and generally yields lower error, the other one is a local optimum
 * outside the field. One could exclude the out-of-field solution with some mildly expensive modifications to the
 * optimization's error function, but the out-of-field scenario is not unrealistic, so this is not the way. Adding an
 * external z source could help increasing the error of the wrong local optimum, but it may not be enough. Another
 * shortcoming of this scenario is when we see the goalposts from the opposite goal, creating large visual error.
 *
 ...
 *
 * 		short line (length < 0.5m) *hard to detect orientation, *generated displacement error is insuficient for
 * optimization long line (length >= 0.5m)
 *
 ...
 * 				- longest line must be >1.6m so that we can extract the orientation (hor/ver) while not being mistaken
 * for a ring line
 ...
 *                  - If the agent sees 1 landmark: compute XY translation for each of the 4 orientations based on that
 * 1 landmark
 ...
 *                      NOTE: If there is 1 landmark, plausibility is defined by mapping errors, not distance to last
 * known pos. So if one guess has the only acceptable mapping error, but is the farthest from previous position, it is
 * still chosen. However, if >1 guess has acceptable mapping error, the 0.5m threshold is used to eliminate candidates.
 *                  Likely if:
 *                      - Plausible
 ...
 * =====================================GRADIENT======================================
 *
 * Grad(PDF[En](p,on,mn,estz)) wrt p = Grad( PDF[E1](p,o1,m1) * ... * PDF[E2](p,on,mn) * PDF[zE](pz,estz)) wrt
 * {px,py,pz}
 *
...
class LocalizerV2
{
  ...
  private:
    ...
    double errorSum_fineTune_before[7] = {
        0}; //[0,1,2]- xyz err sum, [3]-2D err sum, [4]-2D err sq sum, [5]-3D err sum, [6]-3D err sq sum
    double errorSum_fineTune_euclidianDist[7] = {
        0}; //[0,1,2]- xyz err sum, [3]-2D err sum, [4]-2D err sq sum, [5]-3D err sum, [6]-3D err sq sum
    double errorSum_fineTune_probabilistic[7] = {
        0}; //[0,1,2]- xyz err sum, [3]-2D err sum, [4]-2D err sq sum, [5]-3D err sum, [6]-3D err sq sum
    double errorSum_ball[7] = {
        0}; //[0,1,2]- xyz err sum, [3]-2D err sum, [4]-2D err sq sum, [5]-3D err sum, [6]-3D err sq sum
