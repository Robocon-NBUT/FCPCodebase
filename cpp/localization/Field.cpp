...
void Field::draw_visible(const Matrix4D &headToFieldT, bool is_right_side) const
{
    ...
    for (const Line6 &l : list_segments)
    {

        Vector3 s = headToFieldT * l.startc;
        Vector3 e = headToFieldT * l.endc;
    ...
    for (const auto &s : list_known_segments)
    {
        ...
        Vector3 mid = Vector3::determineMidpoint(s.point[0].absPos.get_vector(), s.point[1].absPos.get_vector());

        string line_name(s.fieldSegment->name);
        roboviz->drawAnnotation(&line_name, mid.x, mid.y, mid.z, 0, 1, 0, &draw_name);
        roboviz->drawLine(s.point[0].absPos.x, s.point[0].absPos.y, s.point[0].absPos.z, s.point[1].absPos.x,
                          s.point[1].absPos.y, s.point[1].absPos.z, 3, 0, 0.8, 0, &draw_name);
    }
    ...
    for (const auto &m : list_known_markers)
    {
        ...
        roboviz->drawLine(m.absPos.x, m.absPos.y, m.absPos.z, m.absPos.x, m.absPos.y, m.absPos.z + 0.5, 1, 0.8, 0.8,
                          0.8, &draw_name);
    }
    ...
    Vector3 me = headToFieldT.toVector3();
    ...
    /*static Vector3 last_known_ball_pos = Vector3();
    if(world.ball_seen){
        last_known_ball_pos = headToFieldT * world.ball_rel_pos_cart;
    }

    Vector3 &b = last_known_ball_pos;
...
void Field::gather_ground_markers()
{
    ...
    for (int i = 0; i < 2; i++)
    {
        if (world.foot_touch[i])
        { // if this foot is touching the ground

            // Vector3 contactAux = world.foot_contact_pt[i]; //contact point using strange coordinate system
            // Vector3 contactpt = Vector3(contactAux.y,-contactAux.x,contactAux.z); // fix coordinate system for both
            // feet

            Vector3 relPos = world.foot_contact_rel_pos[i];
    ...
    // Deactivated since it did not produce better results, even when both feet are floating (it was only better when
    // the robot fell)
    ...
    for(int i=0; i<2; i++){
        if( agent::model->getToeTouch(feetSide[i]) ){ //if this foot is touching the ground
            ...
            Vector3 contactAux = agent::model->getToeContact(feetSide[i]); //contact point using strange coordinate
    system Vector3 contactpt = Vector3(contactAux.y,-contactAux.x,contactAux.z); // fix coordinate system for both feet
            Vector3 relPos = agent::model->getRelPositionOfBodyPoint(agent::model->getBodyPart(toePart[i]),contactpt);
    ...
    for (const auto &g : list_ground_markers)
    {
        ...
        float w = 1.f / (1500.f * var); // weight = (1/var)*k   where k is a constant to transform the large weights
                                        // into number of repetitions
...
void Field::update()
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        if (l->pos.x == -15 && l->pos.y == -10)
        {
            l8 = &list_8_landmarks::_corner_mm;
            fp = &cFieldPoints::corner_mm;
        }
        else if (l->pos.x == -15 && l->pos.y == +10)
        {
            l8 = &list_8_landmarks::_corner_mp;
            fp = &cFieldPoints::corner_mp;
        }
        else if (l->pos.x == +15 && l->pos.y == -10)
        {
            l8 = &list_8_landmarks::_corner_pm;
            fp = &cFieldPoints::corner_pm;
        }
        else if (l->pos.x == +15 && l->pos.y == +10)
        {
            l8 = &list_8_landmarks::_corner_pp;
            fp = &cFieldPoints::corner_pp;
        }
        else if (l->pos.x == -15 && l->pos.y < 0)
        {
            l8 = &list_8_landmarks::_goal_mm;
            fp = &cFieldPoints::goal_mm;
        }
        else if (l->pos.x == -15 && l->pos.y > 0)
        {
            l8 = &list_8_landmarks::_goal_mp;
            fp = &cFieldPoints::goal_mp;
        }
        else if (l->pos.x == +15 && l->pos.y < 0)
        {
            l8 = &list_8_landmarks::_goal_pm;
            fp = &cFieldPoints::goal_pm;
        }
        else if (l->pos.x == +15 && l->pos.y > 0)
        {
            l8 = &list_8_landmarks::_goal_pp;
            fp = &cFieldPoints::goal_pp;
        }
        else
        {
            return;
        ...
        if (l->seen)
        {
            ...
            if (l->isCorner)
            {
                list_landmarks_corners.push_back(seen_mark);
            }
            else
            {
                list_landmarks_goalposts.push_back(seen_mark);
            }
        }
        else
        {
    ...
    //----------------------------------------- Gather markers with known absolute z: line endpoints + foot contact
    // points
...
void Field::update_from_transformation(const Matrix4D &tmatrix)
{
    ...
    vector<Line6 *> lines_descending_length;
    ...
    sort(lines_descending_length.begin(), lines_descending_length.end(),
         [](const Line6 *a, const Line6 *b) { return (a->length > b->length); });

    //----------------------------------------------- identify lines

    for (const Line6 *l : lines_descending_length)
    {
        Vector3 l_abs[2] = {tmatrix * l->startc, tmatrix * l->endc};
        ...
        for (const auto &s : cFieldLineSegments::list)
        { // find distance to closest field line
            ...
            if (line_angle_difference > 0.26)
                continue; // tolerance 15deg
            ...
            if (err < min_err)
                err += fieldLineSegmentDistToCart2DPoint(s, l_abs[1].to2d());
            ...
            if (err < min_err)
            {
                ...
                {
                    best_line = &s;
                } // Save the field line for now (others may emerge)
                else
                {
        ...
        if (best_line != nullptr)
        {
            ...
            if (normalize_vector_angle_rad(l_angle - best_line->angle) > 1.57079633f)
            { // they point in opposite directions
                l_index[0] = 1;
                l_index[1] = 0;
            }

            const Vector3 *lAbs[2] = {&l_abs[l_index[0]], &l_abs[l_index[1]]};
            const Vector3 *lRelP[2] = {&l->get_polar_pt(l_index[0]), &l->get_polar_pt(l_index[1])};
            const Vector3 *lRelC[2] = {&l->get_cart_pt(l_index[0]), &l->get_cart_pt(l_index[1])};
             ...
             *            which goes with the theory that the FoV is actually defined as a cone (a bit unrealistic
             * though) current solution: cone_angle < hor_FoV-5
             */
            ...
            // 2nd: use real coordinates if point was recognized, otherwise push it to a valid position (& save segment
            // and unknown markers)
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                {
                    Vector3 p =
                        field_line.segmentPointClosestToCartPoint(*lAbs[i]); // push point to closest valid position
            ...
            list_known_segments.emplace_back(sMarker(l_pt_d[0], *lRelP[0], *lRelC[0]),
                                             sMarker(l_pt_d[1], *lRelP[1], *lRelC[1]), l->length, best_line);
        }
    }
}
...
void Field::update_unknown_markers(const Matrix4D &tmatrix)
{
    ...
    for (auto &u : list_unknown_markers)
    {
        ...
        Vector3 raw_abs_pos = tmatrix * u.relPosCart;
 ...
 */
float Field::fieldLineSegmentDistToCartPoint(const sFieldSegment &fLine, const Vector3 &cp)
{
    ...
    Vector3 w1(cp.x - fLine.point[0]->pt.x, cp.y - fLine.point[0]->pt.y, cp.z); // vector: (segment start -> point)
    if (w1.x * vx + w1.y * vy <= 0)
        return w1.length(); // if angle between vectors is >=90deg, we return the distance to segment start

    Vector3 w2(cp.x - fLine.point[1]->pt.x, cp.y - fLine.point[1]->pt.y, cp.z); // vector: (segment end -> point)
    if (w2.x * vx + w2.y * vy >= 0)
        return w2.length(); // if angle between vectors is <=90deg, we return the distance to segment end

    Vector3 v_cross_w1(vy * w1.z, -vx * w1.z, vx * w1.y - vy * w1.x);
    return v_cross_w1.length() /
