...
#define L11                                                                                                            \
    S98, -2, -3, -3, S19, -3, -3, -2,                                                                                  \
        S98                        // Line 11:soft W, empty field, goal post, soft W, goal post,empty field, soft W
#define L12 S11, -2, E197, -2, S11 // Line 12:soft W, empty field, soft W

#define L12x33                                                                                                         \
    L12, L12, L12, L12, L12, L12, L12, L12, L12, L12, L12, L12, L12, L12, L12, L12, L12, L12, L12, L12, L12, L12, L12, \
        L12, L12, L12, L12, L12, L12, L12, L12, L12, L12
#define LIN12_308 L12x33, L12x33, L12x33, L12x33, L12x33, L12x33, L12x33, L12x33, L12x33

#define L309                                                                                                           \
    S98, -2, -3, -3, E19, -3, -3, -2,                                                                                  \
        S98 // Line 309: soft W, empty field, goal post, empty field, goal post,empty field, soft W
...
Node *pop(Node *root)
{
    ...
    {
        if (MIN == root)
        {                   //------(A)------ min node is root and has no children
    ...
    {
        if (MIN == root)
        { //------(C)------ min node is root and has right child
            MIN = MIN->right;
            root = MIN;
            root->up = nullptr;
...
Node *delete_node(Node *node, Node *root)
{
    ...
    { //------(A)------ node has no children (it can't be root, otherwise it would be min node)
    ...
    { //------(B)------ node has right child (it can't be root, otherwise it would be min node)
    ...
    { //------(D)------ node has 2 children
        ...
        { //----- if successor is the node's right child (successor has no left child)
            ...
            if (node->up->left == node)
            {
                node->up->left = successor;
            }
            else
            {
                node->up->right = successor;
            }
        }
        else
        { //------ if successor is deeper (successor has no left child, and is itself a left child)
            do
            {
                          ...
                          float extra)
{
    ...
    float g = curr_node->g + extra + fmaxf(0.f, cost); // current cost + child distance
    ...
    if (state)
    {
        ...
        {
            open_root = open::delete_node(
                child,
                open_root); // if it is an improvement: remove reference, update it, add it again in correct order
        }
    }
    else
    {
        node_state[pos] = State::Open;
    }
...
inline void build_final_path(Node *const best_node, const Node *board, float status, const bool override_end = false,
                             const float end_x = 0, const float end_y = 0)
{
    // Using 'current_node' would suffice if A* reaches the objective (but 'best_node' works with impossible paths or
    // timeout)
    Node *ptr = best_node;
    int counter = 0;
    do
    ...
    final_path[final_path_size++] =
        status; // 0-success, 1-timeout, 2-impossible, 3-no obstacles(this one is not done in this function)
...
inline bool does_intersect_any_goal(float a_x, float a_y, float b_x, float b_y)
{
    ...
    if (ab_x != 0)
    { // Check if 'ab' and goal back is noncollinear (collinear intersections are ignored)

        k = (15.75 - a_x) / ab_x; // a_x + ab_x*k = 15.75
        if (k >= 0 and k <= 1 and fabsf(a_y + ab_y * k) <= 1.25)
        { // collision (intersection_y = a_y + ab_y*k)
            return true;
        }
    ...
    if (ab_y != 0)
    { // Check if 'ab' and goal sides are noncollinear (collinear intersections are ignored)

        k = (1.25 - a_y) / ab_y; // a_y + ab_y*k = 1.25
        if (k >= 0 and k <= 1)
        {
            float intersection_x_abs = fabsf(a_x + ab_x * k);
            if (intersection_x_abs >= 15 and intersection_x_abs <= 15.75)
            { // check one side for both goals at same time
                return true;
            }
        }
                        ...
                        int given_obst_size, bool go_to_goal, int wall_index, vector<double>& board_cost)
{
    ...
    // Let A* handle it if the start position is unreachable or (out of bounds is not allowed && is nearly out of bounds
    // && not near end)
     ...
     * values explanation:
     * - goal post location (tested in simulator, collision with robot, robot is sideways to the goal post and arms are
     * close to body)
    ...
    //------------------------ Special case (start ~= end): the path is obstructed if start or end are inside any hard
    // circumference
    ...
    if (is_near)
    {
        for (int ob = 0; ob < obst_size; ob += 4)
        {
            ...
            if (sc_x * sc_x + sc_y * sc_y <= r_sq or ec_x * ec_x + ec_y * ec_y <= r_sq)
            { // check distance: center<->start center<->end
                return true;
            }
        }
    }
    else
    {
        ...
        //-------------------- Normal case (start !~= end): the path is obstructed if it intersects any hard or soft
        // circumference
        ...
        for (int ob = 0; ob < obst_size; ob += 4)
        {
            ...
            float sc_proj_scale = (sc_x * se_x + sc_y * se_y) /
                                  (se_x * se_x + se_y * se_y); // scale = projection length / target vector length
            ...
            float k = abs(se_x) > abs(se_y)
                          ? sc_proj_x / se_x
                          : sc_proj_y / se_y; // we use the largest dimension of start->end to avoid division by 0

            if (k <= 0)
            {
                if (sc_x * sc_x + sc_y * sc_y <= r_sq)
                { // check distance: center<->start
                    return true;
                }
            }
            else if (k >= 1)
            {
                float ec_x = c_x - end_x;
                float ec_y = c_y - end_y;
                if (ec_x * ec_x + ec_y * ec_y <= r_sq)
                { // check distance: center<->end
                    return true;
                }
            }
            else
            {
                ...
                if (proj_c_x * proj_c_x + proj_c_y * proj_c_y <= r_sq)
                { // check distance: center<->projection
                    return true;
                }
            }
        }
    ...
    final_path[5] =
        sqrtf(path_x * path_x + path_y * path_y) +
        max(0.f,
            e_cost /
                10.f); // min. A* cost from start to end (e_cost is added even if start==end to help debug cell costs)
...
void astar(float params[], int params_size)
{
    ...
    vector<double> board_cost = {L0_1, L2_5, L6_10, L11, LIN12_308, L309, L310_314, L2_5, L0_1};

    if (!allow_out_of_bounds)
    { // add cost to getting near sideline or endline (except near goal)
    ...
    // define board limits considering the initial and final positions (and obstacles in the next section, and goals
    // after that)
    ...
    if (!allow_out_of_bounds)
    { // workspace must contain a bit of empty field if out of bounds is not allowed
        l_min = min(l_min, 306);
        l_max = max(14, l_max);
        c_min = min(c_min, 206);
        c_max = max(14, c_max);
    ...
    vector<State> node_state(LINES * COLS);
    ...
    for (int ob = 0; ob < obst_size; ob += 5)
    {
        ...
        int max_r = int(fmaxf(hard_radius, soft_radius) * 10.f +
                        1e-4); // add epsilon to avoid potential rounding error in expansion groups
    ...
    if (c_max > 96 and c_min < 124)
    { // Otherwise it does not overlap any goal
        if (l_max > 1 and l_min < 12)
        {                           // Overlaps our goal
    ...
    board[start_pos].g = 0; // This is needed to compute the cost of child nodes, but f is not needed because there are
                            // no comparisons with other nodes in the open BST
    ...
    Node *best_node =
        &board[start_pos]; // save best node based on distance to goal (useful if impossible/timeout to get best path)

    float best_node_dist = numeric_limits<float>::max(); // infinite distance if start is itself unreachable
    ...
    while (open_root != nullptr)
    {
        ...
        node_state[curr_pos] = State::Closed;

        // Check if we reached objective
        if (curr_cost == -1)
        {
            // replace end point with correct coordinates instead of discrete version if the optional target was defined
            // (not going to goal)
            build_final_path(best_node, board, 0, !go_to_goal, opt_t_x, opt_t_y);
            return;
        }
        if (measure_timeout == 0 and
            duration_cast<chrono::microseconds>(chrono::high_resolution_clock::now() - t1).count() > timeout_us)
        {
        ...
        if (curr_line > l_min)
        {
            ...
            auto state = node_state[pos];

            // check if not an obstacle and if node is not closed (child can be as inaccessible as current pos)
            if (state != State::Closed and !(cost <= wall_index and cost < curr_cost) and lcol_ok)
            {
                open_root = expand_child(open_root, cost, wall_index, curr_node, board, pos, state, go_to_goal, line,
                                         col, end_l, end_c, node_state, numbers::sqrt2);
            }
            ...
            // check if not an obstacle and if node is not closed (child can be as inaccessible as current pos)
            if (state != State::Closed and !(cost <= wall_index and cost < curr_cost))
            {
                open_root = expand_child(open_root, cost, wall_index, curr_node, board, pos, state, go_to_goal, line,
                                         col, end_l, end_c, node_state, 1);
            }
            ...
            // check if not an obstacle and if node is not closed (child can be as inaccessible as current pos)
            if (state != State::Closed and !(cost <= wall_index and cost < curr_cost) and rcol_ok)
            {
                open_root = expand_child(open_root, cost, wall_index, curr_node, board, pos, state, go_to_goal, line,
                                         col, end_l, end_c, node_state, numbers::sqrt2);
            }
...

        {
            ...
            if (state != State::Closed and !(cost <= wall_index and cost < curr_cost) and lcol_ok)
            {
                open_root = expand_child(open_root, cost, wall_index, curr_node, board, pos, state, go_to_goal,
                                         curr_line, col, end_l, end_c, node_state, 1);
            }
