...
class UI:
    ...
    @staticmethod
    def input_num(prompt: str, min_num: int, max_num: int):
        """
        输入一个数字，确保在 [min_num. max_num) 之间
        """
        while True:
            inp = input(prompt)
            try:
    ...
    def print_table(data, titles=None, alignment=None, cols_width=None, cols_per_title=None, margins=None, numbering=None, prompt=None):
        ...
        * if `numbering` or `prompt` are `None`, `None` is returned
            ...
            if cols_width[i] == 0:
                ...
                if cols_per_title is None or cols_per_title[i] < 2:
                    ...
                    for j in range(cols_per_title[i]):
                        subcol_data_width = max(
                            [len(str(item))+numbering_width for item in data[i][j*subcol[i][0]:j*subcol[i][0]+subcol[i][0]]])
                        # add subcolumn data width to column width
                        cols_width[i] += subcol_data_width
                        # save subcolumn data width
                        subcol[i][1].append(subcol_data_width)

                if titles is not None:  # expand to acomodate titles if needed
                    cols_width[i] = max(
                        cols_width[i], len(titles[i]) + margins[i]*2)
        ...
        if any_numbering:
            ...
            for i in range(cols_no):
                assert isinstance(
                    data[i], list), "In function 'print_table', 'data' must be a list of lists!"

                if numbering[i]:
                    data[i] = [f"{n+no_of_items:3}-{d}" for n,
                               d in enumerate(data[i])]
        ...
        if titles is not None:
            for i in range(cols_no):
                print(f'{titles[i]:^{cols_width[i]}}',
                      end='|' if i < cols_no - 1 else '')
            print()
            for i in range(cols_no):
                print(f'{"-"*cols_width[i]}',
                      end='+' if i < cols_no - 1 else '')
            print()
        ...
        for line in zip_longest(*data):
            for i, item in enumerate(line):
                ...
                l_margin = margins[i]-1 if numbering[i] else margins[i]
    ...
    def print_list(data, numbering=True, prompt=None, divider=" | ", alignment="<", min_per_col=6):
        ...
        for row in range(np.ceil(data_size / i)):
            for col in range(i):
                ...
                content = cols_items[col][row] if len(
                    cols_items[col]) > row else divider
                if col == 0:
                    l = len(divider)
                    # remove divider from 1st col
                    print(end=f"{content[l:]:{alignment}{cols_width[col]-l}}")
                else:
        ...
        if numbering is None:
            ...
            idx = UI.input_num(prompt, 0, data_size)
            return idx, data[idx]
