...
class IMU():
    ...
    def __init__(self, script: Script) -> None:
        ...
        self.colors = [Draw.Color.green_light,
                       Draw.Color.yellow, Draw.Color.red]
    ...
    def act(self):
        ...
        self.cycle += 1
        if self.cycle < 50:
            r.set_joints_target_position_direct(joint_indices, np.array(
                [32+10, -64, 32, 45,  40+10, -80, 40,  0])*amplitude)
        elif self.cycle < 100:
            r.set_joints_target_position_direct(joint_indices, np.array(
                [-10,  0, 0,  0,    -10,  0, 0,  0])*amplitude)
        elif self.cycle < 150:
            r.set_joints_target_position_direct(joint_indices, np.array(
                [40+10, -80, 40,  0,  32+10, -64, 32, 45])*amplitude)
        elif self.cycle < 200:
            r.set_joints_target_position_direct(joint_indices, np.array(
                [-10,  0, 0,  0,    -10,  0, 0,  0])*amplitude)
        else:
    ...
    def act2(self):
        ...
        self.player.behavior.execute("Walk", (0.2, 0), False, 5, False, None)
        self.player.server.commit_and_send(r.get_command())
        self.player.server.receive()
    ...
    def compute_local_IMU(self):
        ...
        self.imu_torso_acceleration[2] = self.imu_torso_to_field_rotation[2].multiply(
            r.acc) + Robot.GRAVITY
        ...
        self.imu_CoM_position[2][:] = self.imu_head_to_field_transform[2](
            r.rel_cart_CoM_position)

        # Next Position = x0 + v0*t + 0.5*a*t^2,   Next velocity = v0 + a*t
        self.imu_torso_next_position[2] = self.imu_torso_position[2] + \
            self.imu_torso_velocity[2] * 0.02 + \
            self.imu_torso_acceleration[2] * 0.0002
        self.imu_torso_next_velocity[2] = self.imu_torso_velocity[2] + \
            self.imu_torso_acceleration[2] * 0.02
        # stability tradeoff
    ...
    def compute_local_IMU_rotation_only(self):
        ...
        self.imu_head_to_field_transform[1] = self.imu_torso_to_field_transform[1].multiply(
            r.body_parts["torso"].transform.invert())
        self.imu_CoM_position[1][:] = self.imu_head_to_field_transform[1](
            r.rel_cart_CoM_position)
    ...
    def execute(self):
        ...
        self.player.server.unofficial_beam(
            (-3, 0, self.player.world.robot.beam_height), 15)
