...
class Fwd_Kinematics:
        ...
        for _ in range(self.cycle_duration):
            ...
            for p in self.script.players:
                ...
                for index, joint in enumerate(p.world.robot.joints):
                    ...
                    p.world.draw.annotation(
                        label, j_name, Draw.Color.cyan, j_name)
            ...
            for p in self.script.players:
                ...
                for key in p.world.robot.body_parts:
                    ...
                    if key not in ['head', 'torso', 'llowerarm', 'rlowerarm', 'lthigh', 'rthigh', 'lshank', 'rshank', 'lfoot', 'rfoot']:
                        continue
                    bpart_abs_pos = p.world.robot.get_body_part_to_field_transform(
                        # 10cm in front of body part
                        key).translate((0.1, 0, 0))
                    ...
                    p.world.draw.line(axes_0, x_axis, 2,
                                      Draw.Color.green_light, key, False)
                    p.world.draw.line(axes_0, y_axis, 2,
                                      Draw.Color.blue, key, False)
    ...
    def execute(self):
        ...
        # Beam players
        self.script.batch_unofficial_beam(
            [(-2, i*4-10, 0.5, i*45) for i in range(5)])
