...
class Radio_Localization():
    ...
    def draw_objects(self, p: Agent, pos, is_down, was_seen, last_update, is_self=False):
        ...
        if is_self:
            ...
            if is_down:
                d.annotation((me[:2]+pos[:2])/2, "Fallen",
                             d.Color.white, "objects", False)
    ...
    def execute(self):
        ...
        self.script.batch_create(
            Agent, ((a.i, a.p, a.m, u, a.t,       False, u == 1) for u in range(1, 12)))
        self.script.batch_create(
            Agent, ((a.i, a.p, a.m, u, "Opponent", False, False) for u in range(1, 12)))
        ...
        # Execute
        for j in count():
            # run our agents (think and send)
            self.script.batch_execute_agent(slice(11))
            # run their agents (don't think, just send)
            self.script.batch_commit_and_send(slice(11, None))

            # draw knowledge, iterate through our team, 15 time steps per player
            self.draw(players[j//15 % 11])
            # receive & update our team's world state
            self.script.batch_receive(slice(11))
            # receive & don't update opponent's world state (to save cpu resources)
            self.script.batch_receive(slice(11, None), False)
