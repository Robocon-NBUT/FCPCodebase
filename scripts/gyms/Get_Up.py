...
class Get_Up(gym.Env):
    def __init__(self, ip, server_p, monitor_p, r_type, fall_direction, enable_draw) -> None:
        ...
        # one-hot encoding for each slot
        self.obs = np.identity(len(self.original_slots))
        self.current_slot = 0

        MAX = np.finfo(np.float32).max
        self.action_space = gym.spaces.Box(low=np.full(
            11, -MAX, np.float32), high=np.full(11, MAX, np.float32), dtype=np.float32)
        self.observation_space = gym.spaces.Box(low=np.zeros(len(
            self.obs), np.float32), high=np.ones(len(self.obs), np.float32), dtype=np.float32)
    ...
    def fall(self):
        ...
        if self.fall_direction == 0:
            r.set_joints_target_position_direct(
                joint_indices, np.array([50, 50, 0, 0]))
        elif self.fall_direction == 1:
            r.set_joints_target_position_direct(
                joint_indices, np.array([0, 0, -20, 20]))
        elif self.fall_direction == 2:
            r.set_joints_target_position_direct(
                joint_indices, np.array([0, 0, 20, -20]))
        elif self.fall_direction == 3:
            r.set_joints_target_position_direct(
                joint_indices, np.array([-20, -20, 0, 0]))
        else:
            raise ValueError
    ...
    def get_up(self):
        r = self.player.world.robot
        finished = self.player.behavior.execute(
            self.get_up_names[self.fall_direction])
    ...
    def reset(self):
        self.player.server.commit_beam((-3, 0), 0)
        ...
        t = random.randint(
            7, 17) if self.fall_direction == 0 else random.randint(10, 20)
        for _ in range(t):
            self.other("Zero")
    ...
    def get_22_angles(angles, indices):
        ...
        # get all joints that are defined in the XML (otherwise, assume 0)
        new_angles[indices] = angles
...
class Train(Train_Base):
    ...
    def train(self, args):
        ...
        total_steps = 1000
        learning_rate = 2e-4
        folder_name = f'GetUp_R{self.robot_type}_Direction{
            self.fall_direction}'
        ...
        try:
            ...
            model_path = self.learn_model(
                model, total_steps, model_path, eval_env=eval_env, eval_freq=n_steps_per_env*10, backup_env_file=__file__)
        ...
        self.generate_get_up_behavior(model, model_path, eval_env.get_attr(
            'original_slots')[0], "last_model.xml")
    ...
    def test(self, args):
        ...
        env = Get_Up(self.ip, self.server_p-1, self.monitor_p,
                     self.robot_type, self.fall_direction, True)
        model = PPO.load(args["model_file"], env=env)

        # Generate slot behavior XML
        xml_name = Path(args["model_file"]).stem + ".xml"
        if not (Path(args["folder_dir"]) / xml_name).is_file():
            self.generate_get_up_behavior(
                model, args["folder_dir"], env.original_slots, xml_name)
    ...
    def generate_get_up_behavior(self, model: BaseAlgorithm, folder_dir, original_slots, xml_name):
        ...
        predictions = model.predict(np.identity(
            len(original_slots)), deterministic=True)[0]
        slots = []

        for index, prediction in enumerate(predictions):
            pred = Get_Up.scale_action(prediction)
            delta = max((original_slots[index][0] + pred[0]) // 20 * 20, 20)
            angles = Get_Up.get_22_angles(
                original_slots[index][2], original_slots[index][1])
            angles[2:] += pred[1:]
