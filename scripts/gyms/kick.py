...
class Kick(gym.Env):
    def __init__(self, ip, server_p, monitor_p, r_type, enable_draw) -> None:
        ...
        self.player.server.unofficial_move_ball((0, 0, 0))
    ...
    def observe(self, init=False):
        ...
        ball_rel_hip_center = self.player.inv_kinematics.torso_to_hip_transform(w.Ball.RelativeTorsoCartPos)
        if init:
        ...
        self.obs[62] = normalize_deg(self.kick_ori - r.IMU.TorsoOrientation) / 30
        '''
    ...
    def reset(self):
        ...
        self.player.server.unofficial_move_ball((Gen_ball_pos[0], Gen_ball_pos[1], Gen_ball_pos[2]))
        self.sleep = 0
        self.step_counter = 0
        ...
        for _ in range(25):
            self.player.server.unofficial_beam(Gen_player_pos, 0)  # beam player continuously (floating above ground)
            self.player.behavior.execute("Zero_Bent_Knees")
            self.sync()

        # beam player to ground
        self.player.server.unofficial_beam(Gen_player_pos, 0)
        r.joints[0].target_speed = 0.01  # move head to trigger physics update (rcssserver3d bug when no joint is moving)
        self.sync()
        ...
        while True and w.time_local_ms - self.reset_time <= 50000:
            ...
            biased_dir = normalize_deg(direction + self.bias_dir)  # add bias to rectify direction
            ang_diff = abs(
                normalize_deg(biased_dir - r.location.Torso.Orientation))  # the reset was learned with loc, not IMU
    ...
    def step(self, action):
        r = self.player.world.robot
        b = self.player.world.Ball.AbsolutePos
        w = self.player.world
        t = w.time_local_ms
        for idx, val in enumerate(action):
            r.joints[idx + 2].target_speed = action[idx]
        ...
        if self.step_counter > 22:
            obs = self.observe()
            self.player.server.unofficial_beam((-14.5, 0, 0.51), 0)  # beam player continuously (floating above ground)
            waiting_steps = 0
            high = 0
            while waiting_steps < 650:  # 假设额外等待5个步骤
                if w.Ball.CheatAbsPos[2] > high:
                    high = w.Ball.CheatAbsPos[2]
                self.sync()  # 继续执行仿真步骤
                waiting_steps += 1
            dis = np.linalg.norm(self.ball_pos - w.Ball.CheatAbsPos)
            reward = dis - abs(w.Ball.CheatAbsPos[1] - self.ball_pos[1]) + high*0.2
            # print(reward)
            self.terminal = True
