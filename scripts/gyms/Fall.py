...
class Fall(gym.Env):
    def __init__(self, ip, server_p, monitor_p, r_type, enable_draw) -> None:
        ...
        # joints + torso height
        self.obs = np.zeros(self.no_of_joints + 1, np.float32)
        self.observation_space = gym.spaces.Box(low=np.full(len(
            self.obs), -np.inf, np.float32), high=np.full(len(self.obs), np.inf, np.float32), dtype=np.float32)

        # Action space
        MAX = np.finfo(np.float32).max
        no_of_actions = self.no_of_joints
        self.action_space = gym.spaces.Box(low=np.full(
            no_of_actions, -MAX, np.float32), high=np.full(no_of_actions, MAX, np.float32), dtype=np.float32)

        # Check if cheats are enabled
        assert np.any(
            self.player.world.robot.cheat_abs_pos), "Cheats are not enabled! Run_Utils.py -> Server -> Cheats"
    ...
    def observe(self):
        ...
        self.obs[self.no_of_joints] = r.cheat_abs_pos[2]
    ...
    def reset(self):
        ...
        for _ in range(25):
            ...
            self.player.server.unofficial_beam((-3, 0, 0.50), 0)
    ...
    def step(self, action):
        ...
        if self.obs[-1] < 0.15:           # terminal state: the robot has fallen successfully
            # Reward: 1 (this reward will motivate a fast reaction if the return is discounted)
            return self.obs, 1, True, {}
        # terminal state: 3s passed and robot has not fallen (may be stuck)
...
class Train(Train_Base):
    ...
    def train(self, args):
        ...
        # RolloutBuffer is of size (n_steps_per_env * n_envs) (*RV: >=2048)
        n_steps_per_env = 128
        ...
        try:
            if "model_file" in args:  # retrain
                model = PPO.load(
                    args["model_file"], env=env, n_envs=n_envs, n_steps=n_steps_per_env,
    ...
    def test(self, args):
        ...
        env = Fall(self.ip, self.server_p-1,
