...
class Basic_Run(gym.Env):
    def __init__(self, ip, server_p, monitor_p, r_type, enable_draw) -> None:
        ...
        self.player = Agent(ip, server_p, monitor_p, 1,
                            self.robot_type, "Gym", True, enable_draw)
        self.step_counter = 0  # to limit episode size

        self.step_obj: Step = self.player.behavior.get_custom_behavior_object(
            "Step")  # Step behavior object

        # State space
        obs_size = 70
        self.obs = np.zeros(obs_size, np.float32)
        self.observation_space = gym.spaces.Box(low=np.full(
            obs_size, -np.inf, np.float32), high=np.full(obs_size, np.inf, np.float32), dtype=np.float32)

        # Action space
        MAX = np.finfo(np.float32).max
        self.no_of_actions = act_size = 22
        self.action_space = gym.spaces.Box(low=np.full(
            act_size, -MAX, np.float32), high=np.full(act_size, MAX, np.float32), dtype=np.float32)
        ...
        self.player.server.unofficial_move_ball((14, 0, 0.042))
    ...
    def observe(self, init=False):
        ...
        self.obs[3] = r.IMU.TorsoOrientation / \
            50   # absolute orientation in deg
        self.obs[4] = r.IMU.TorsoRoll / 15   # absolute torso roll  in deg
        self.obs[5] = r.IMU.TorsoPitch / 15   # absolute torso pitch in deg
        self.obs[6:9] = r.gyro / 100  # gyroscope
        self.obs[9:12] = r.acc / 10   # accelerometer

        # left foot: relative point of origin (p) and force vector (f) -> (px,py,pz,fx,fy,fz)*
        self.obs[12:18] = r.frp.get('lf', (0, 0, 0, 0, 0, 0))
        # right foot: relative point of origin (p) and force vector (f) -> (px,py,pz,fx,fy,fz)*
        ...
        if init:
            self.obs[64] = self.step_default_dur / \
                10  # step duration in time steps
            self.obs[65] = self.step_default_z_span * \
                20  # vertical movement span
            # relative extension of support leg
            ...
            self.obs[64] = self.step_obj.step_generator.ts_per_step / \
                10  # step duration in time steps
            # vertical movement span
            self.obs[65] = self.step_obj.step_generator.swing_height * 20
            self.obs[66] = self.step_obj.step_generator.max_leg_extension / \
                self.step_obj.leg_length  # relative extension of support leg
            # step progress
            self.obs[67] = self.step_obj.step_generator.external_progress
            # 1 if left  leg is active
            self.obs[68] = float(
                self.step_obj.step_generator.state_is_left_active)
            # 1 if right leg is active
            self.obs[69] = float(
                not self.step_obj.step_generator.state_is_left_active)
    ...
    def reset(self):
        ...
        for _ in range(25):
            # beam player continuously (floating above ground)
            self.player.server.unofficial_beam((-14, 0, 0.50), 0)
    ...
    def step(self, action):
        ...
        if self.step_counter == 0:
            ...
            self.player.behavior.execute(
                "Step", self.step_default_dur, self.step_default_z_span, self.step_default_z_max)
        else:

            step_zsp = np.clip(self.step_default_z_span +
                               self.act[20]/300,   0,     0.07)
            step_zmx = np.clip(self.step_default_z_max +
                               self.act[21]/30,    0.6,   0.9)

            self.player.behavior.execute(
                "Step", self.step_default_dur, step_zsp, step_zmx)
        ...
        # scale up actions to motivate exploration
...
class Train(Train_Base):
    ...
    def train(self, args):
        ...
        # RolloutBuffer is of size (n_steps_per_env * n_envs)
        n_steps_per_env = 1024
        # should be a factor of (n_steps_per_env * n_envs)
        minibatch_size = 64
        ...
        # include 1 extra server for testing
    ...
    def test(self, args):
        ...
        env = Basic_Run(self.ip, self.server_p-1,
                        self.monitor_p, self.robot_type, True)
        model = PPO.load(args["model_file"], env=env)

        try:
            # Export to pkl to create custom behavior
            self.export_model(args["model_file"],
                              args["model_file"]+".pkl", False)
            self.test_model(
                model, env, log_path=args["folder_dir"], model_path=args["folder_dir"])
        except KeyboardInterrupt:
            print()
