...
class Get_Up():
    ...
    def execute(self, reset):
        ...
        if self.state == 0:  # State 0: go to pose "Zero"
            ...
            self.watchdog += 1
            # log last STABILITY_THRESHOLD values
            self.gyro_queue.append(max(abs(r.gyro)))
            ...
            if (execute_sub_behavior("Zero", None) and len(self.gyro_queue) == self.STABILITY_THRESHOLD
                ...
                if r.acc[0] < -4 and abs(r.acc[1]) < 2 and abs(r.acc[2]) < 3:
                    execute_sub_behavior(
                        "Get_Up_Front", True)  # reset behavior
                    self.state = 1
                elif r.acc[0] > 4 and abs(r.acc[1]) < 2 and abs(r.acc[2]) < 3:
                    execute_sub_behavior("Get_Up_Back", True)  # reset behavior
                    self.state = 2
                # fail-safe if vision is not up to date: if pose is 'Zero' and torso is upright, the robot is already up
                elif r.acc[2] > 8:
                    return True
                else:
    ...
    def is_ready(self):
        ...
        return r.acc[2] < 5 and np.dot(r.acc, r.acc) > 64 and (r.location.Head.Head_Z < self.MIN_HEIGHT or r.IMU.TorsoInclination > self.MAX_INCLIN)
