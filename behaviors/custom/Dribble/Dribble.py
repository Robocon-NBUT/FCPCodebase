...
class Dribble():
    ...
    def define_approach_orientation(self):
        ...
        # safety margin (if ball is near the field limits by this amount, the approach orientation is considered)
        MARGIN = 0.8
        ...
        self.approach_orientation = a1 if a1_diff < a2_diff else a2
        ...
        if self.phase == 0:  # walk to ball
            ...
            #  2C. A better approach orientation is not needed and the robot can see the ball
                ...
                if 0.18 < b_rel[0] < 0.25 and abs(b_rel[1]) < 0.05 and w.Ball.IsVisible:
                    ...
                    rel_target = b_rel+(-0.23, 0)
                    # relative orientation of ball, NOT the target!
                    rel_ori = vector_angle(b_rel)
                    dist = max(0.08, np.linalg.norm(
                        rel_target)*0.7)  # slow approach
                    # target, is_target_abs, ori, is_ori_abs, distance
                    behavior.execute_sub_behavior(
