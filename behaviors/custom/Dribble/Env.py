...
class Env():
    def __init__(self, base_agent: Base_Agent, step_width) -> None:
        ...
        # relative to imu_torso_orientation (in degrees)
        self.dribble_rel_orientation = None
    ...
    def observe(self, init=False, virtual_ball=False):
        ...
        if init:
            ...
            # for training purposes (reward)
            self.gym_last_internal_abs_ori = r.IMU.TorsoOrientation

        # ---------------------------------------------------------------- compute internal target

        if w.vision_is_up_to_date:

            previous_internal_rel_orientation = np.copy(
                self.internal_rel_orientation)

            internal_ori_diff = np.clip(normalize_deg(
                self.dribble_rel_orientation - self.internal_rel_orientation), -MAX_ROTATION_DIFF, MAX_ROTATION_DIFF)
            ...
            # Observations
            self.internal_target_vel = self.internal_rel_orientation - \
                previous_internal_rel_orientation

            self.gym_last_internal_abs_ori = self.internal_rel_orientation + r.IMU.TorsoOrientation
