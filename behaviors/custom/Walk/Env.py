...
class Env():
    def __init__(self, base_agent: Base_Agent) -> None:
        ...
        # upper leg height + lower leg height
        ...
        self.step_generator = Step_Generator(
            feet_y_dev, sample_time, max_ankle_z)
    ...
    def observe(self, init=False):
        ...
        self.obs[11:17] = r.frp.get('lf', np.zeros(
            6)) * (10, 10, 10, 0.01, 0.01, 0.01)
        # right foot: relative point of origin (p) and force vector (f) -> (px,py,pz,fx,fy,fz)*
        self.obs[17:23] = r.frp.get('rf', np.zeros(
            6)) * (10, 10, 10, 0.01, 0.01, 0.01)
        ...
        # ankle position relative to center of both hip joints
        rel_lankle = self.ik.get_body_part_pos_relative_to_hip("lankle")
        # ankle position relative to center of both hip joints
        rel_rankle = self.ik.get_body_part_pos_relative_to_hip("rankle")
        # foot transform relative to torso
        lf = r.head_to_body_part_transform(
            "torso", r.body_parts['lfoot'].transform)
        # foot transform relative to torso
        rf = r.head_to_body_part_transform(
            "torso", r.body_parts['rfoot'].transform)
        lf_rot_rel_torso = np.array([lf.get_roll_deg(), lf.get_pitch_deg(
        ), lf.get_yaw_deg()])  # foot rotation relative to torso
        rf_rot_rel_torso = np.array([rf.get_roll_deg(), rf.get_pitch_deg(
        ), rf.get_yaw_deg()])  # foot rotation relative to torso
        ...
        if rel_raw_target_size == 0:
            ...
            rel_target = self.walk_rel_target / rel_raw_target_size * \
                min(self.walk_distance, MAX_LINEAR_DIST)
        ...
        if internal_diff_size > MAX_LINEAR_DIFF:
            self.internal_target += internal_diff * \
                (MAX_LINEAR_DIFF / internal_diff_size)
        else:
    ...
    def execute_ik(self, l_pos, l_rot, r_pos, r_rot):
        ...
        r.set_joints_target_position_direct(
            indices, self.values_l, harmonize=False)
        ...
        r.set_joints_target_position_direct(
            indices, self.values_r, harmonize=False)
    ...
    def execute(self, action):
        ...
        action_mult = 1 if internal_dist > 0.2 else (
        ...
        # Leg IK
        a = self.act
        l_ankle_pos = (a[0]*0.02, max(0.01,  a[1]*0.02 + lfy),
                       a[2]*0.01 + lfz)  # limit y to avoid self collision
        r_ankle_pos = (a[3]*0.02, min(a[4]*0.02 + rfy, -0.01),
                       a[5]*0.01 + rfz)  # limit y to avoid self collision
