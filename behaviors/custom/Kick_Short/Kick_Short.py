import pickle
from behaviors.custom.Kick_Short.Env import Env
...
class Kick_Short:
    def __init__(self, base_agent: Base_Agent) -> None:
        ...
        with open(get_active_directory([
            '/behaviors/custom/Kick_Short/short_kick_R0.pkl',
            '/behaviors/custom/Kick_Short/short_kick_R1.pkl',
            '/behaviors/custom/Kick_Short/short_kick_R2.pkl',
            '/behaviors/custom/Kick_Short/short_kick_R3.pkl',
                '/behaviors/custom/Kick_Short/short_kick_R4.pkl'][self.world.robot.type]), "rb") as f:
            self.model = pickle.load(f)

    def execute(self, reset, orientation, distance, wait=False):
        """
        Parameters
        ----------
        orientation : float
        ...
        if reset:
            ...
            dist_factor = 0
            if dist_to_final_target < 0.032 and ang_diff < 6 and (step_gen.state_is_left_active or step_gen.state_current_ts) == 2 and w.time_local_ms - w.Ball.AbsolutePosLastUpdate < 100 and (wait or w.time_local_ms - self.reset_time) > 500:
                self.phase += 1
                reset_kick = True
            else:
                dist_factor = 1.0
                ...
                #         dist_factor = max(
                #             min(1.0, 1.0 - w.front_standing_opp_d * 0.08), 0.8)
            dist = max(0.07, dist_to_final_target * dist_factor)
            d = w.draw
            # d.annotation((*r.loc_head_position[None[:2]], *(1.2, )), f"{w.front_standing_opp_d} {dist_factor}", d.Color.red, "role")
